...
export default function AddScreen() {
  ...
  const [isExerciseSearchModalVisible, setIsExerciseSearchModalVisible] =
    useState(false);
  ...
  // Editing state
  const [currentEditingWorkout, setCurrentEditingWorkout] =
    useState<WorkoutExerciseDTO | null>(null);
  const [editingSetIndex, setEditingSetIndex] = useState<number | null>(null);
  const [editingField, setEditingField] = useState<"weight" | "reps" | null>(
    null,
  );
  const [wheelValue, setWheelValue] = useState(0);
  const [pendingSet, setPendingSet] = useState<Partial<WorkoutSetDTO> | null>(
    null,
  );
  ...
  const handleSaveWorkout = useCallback(async () => {
    ...
    if (!allHaveSets) {
      const exercisesWithoutSets = exercises.filter(
        (ex) => ex.sets.length === 0,
      );
      Alert.alert(
  ...
  const handleExerciseCreated = useCallback(async () => {
    ...
    const newExercise = allExercises.reduce(
      (max, ex) => (ex.id > max.id ? ex : max),
      allExercises[0],
    );
    if (newExercise) {
  ...
  const handleAddSet = useCallback(
    (workoutExerciseId: number) => {
      const workoutExercise = exercises.find(
  ...
  const handleDuplicateSet = useCallback(
    (exerciseId: number, set: WorkoutSetDTO) => {
  ...
  const updateExerciseSets = useCallback(
    (
      exerciseId: number,
      updateFn: (sets: WorkoutSetDTO[]) => WorkoutSetDTO[],
    ) => {
      setExercises((prev) =>
        prev.map((exercise) =>
          exercise.id === exerciseId
            ? { ...exercise, sets: updateFn(exercise.sets) }
            : exercise,
        ),
      );
    },
  ...
  const handleShowExerciseInfo = useCallback(
    (workoutExercise: WorkoutExerciseDTO) => {
  ...
  const handleDuplicateExerciseSets = useCallback(
    (exerciseId: number, sets: WorkoutSetDTO[]) => {
